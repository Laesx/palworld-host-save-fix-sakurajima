...
# WORKAROUND FOR SAKURAJIMA UPDATE
custom_properties_keys = [".worldSaveData.GroupSaveDataMap",
                          ...
                          ".worldSaveData.BaseCampSaveData.Value.ModuleMap", ".worldSaveData.WorkSaveData"]
custom_properties = {}
...
def main():
    ...
    if new_guid[-4:] == '.sav' or old_guid[-4:] == '.sav':
        print(
            'ERROR: It looks like you\'re providing the whole name of the file instead of just the GUID. For example, instead of using "<GUID>.sav" in the command, you should be using only the GUID.')
        exit(1)
...
of your save folder before continuing. Press enter if you would like to continue.')
    ...
    for i in range(instance_ids_len):
        instance_id = \
        level_json['properties']['worldSaveData']['value']['CharacterSaveParameterMap']['value'][i]['key'][
            'InstanceId']['value']
        if instance_id == old_instance_id:
            level_json['properties']['worldSaveData']['value']['CharacterSaveParameterMap']['value'][i]['key'][
                'PlayerUId']['value'] = new_guid_formatted
            break
    ...
    #TODO This one might be optional and should not have custom properties?
